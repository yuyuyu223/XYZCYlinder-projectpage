...
<template>
  <div>
    ...
    <el-col  :span="18">
        <p>
            <!-- 我们使用了SyntheOcc训练了基于高斯语义渲染MPI引导的2D多视角controlnet，并将它输入到我们的模型，下面的视频展示了模型的模型在生成数据上的泛化效果 -->
             To enable controllable view synthesis, we utilized <span style="background: linear-gradient(45deg, #8B4513, #DAA520); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: bold;">SyntheOcc</span> and trained a ControlNet model conditioned on <span style="background: linear-gradient(45deg, #8B4513, #DAA520); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: bold;">semantically rendered Gaussian-based MPIs</span>. The output of this ControlNet serves as a key input to our proposed framework. The video below showcases the excellent generalization performance of our method on generated data.
        </p>
      </el-col>
    </el-row>
